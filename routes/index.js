	...
	User.getUserByUsername(username, function (err, user) {
		...
		User.comparePassword(password, user.password, function(err, isMatch) {
			...
			if (isMatch) {
				...
				return done(null, false, { message: 'Wrong Username or Password.' }); // лучше писать тут что не правильный или логин или пароль вместе, а не только неправильный пароль, что бы нельзя было подбирать, так как если вы пишите что неправильный именно пароль, то человек понимает что логин то правильный, и остаётся подобрать только пароль, а так у нас неопределённость, или то или то неправльно, мы точно не указываем, пусть угадывают, и ещё эти сообщения выводятся в блок с классом .error b его можно стилизоватьв стилях по этому самому классу .error, а выводятся они в флеш-сообщения, кароче в то же место куда и остальные флеш сообщения, в нашем случае у нас шаблонизатор pug, поэтому в место != messages()
			}
		});
	});
}));
